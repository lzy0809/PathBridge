...
- 日期：2026-03-02
  - 用户纠正：Go2Shell 目标架构应为“单安装包交付 1 个 App，但 App 内可以内嵌 Helper”，不是“两个独立 App 并列分发”。
  - 暴露问题：此前把“单架构”误读为“完全单可执行”，导致先做了错误回退，再返工。
  - 预防规则：涉及“对标某应用架构”时，先基于已安装实物包做目录级取证（`Contents/*`）并写入约束，再开始编码。

- 日期：2026-03-02
  - 用户纠正：Finder 工具栏入口图标过小不可接受，必须按 16px 场景优先优化。
  - 暴露问题：此前只做视觉风格选择，缺少小尺寸有效像素占比门禁。
  - 预防规则：Finder helper 图标发布前必须做占比检查（建议 alpha/bbox 占比与参考 Go2Shell 同量级），并用独立素材链路避免被主 App 图标覆盖。
...
- 日期：2026-02-28
  ...
  - 预防规则：凡是“主 App + helper”双角色架构，视觉资源脚本必须同时覆盖两个 target 并在打包前校验哈希一致性。

- 日期：2026-02-28
  - 用户纠正：Kaku 在 macOS 26 上打不开，需要终端适配器做系统差异兼容。
  - 暴露问题：单一启动路径（固定可执行文件 + 固定参数）在不同 Kaku 版本/运行态下不稳定。
  - 预防规则：第三方终端适配必须提供多策略回退（可执行文件候选 + 命令策略候选），并输出结构化日志用于快速定位。

- 日期：2026-02-28
  - 用户纠正：不接受“最小改动”或“兼容式修补”，要求实现思路和行为都按 Go2Shell 对标。
  - 暴露问题：此前给了保守/最小改造建议，和用户“体验完全一致优先”的目标不一致。
  - 预防规则：当用户明确要求“完全对标某产品”时，默认先做行为级与架构级对齐方案，不先给轻量替代路线。

- 日期：2026-02-28
  - 用户纠正：需要区分“Finder 工具栏快开”和“用户手动启动设置”两种入口语义。
  - 暴露问题：此前把 `reopen` 一律当成快开触发，导致 Launchpad 点击也直开终端。
  - 预防规则：`reopen` 触发前必须做上下文门控（至少检查 Finder 是否前台）；非 Finder 场景回到设置 UI。

- 日期：2026-02-27
  - 用户纠正：Finder 工具栏点击触发了 `reopen`，但因 Apple Events 授权被拒（`-1743`）导致无法读取 Finder 当前目录。
  - 暴露问题：主 App 缺少 `NSAppleEventsUsageDescription`，系统不会给授权弹窗；失败时也缺少明确引导。
  - 预防规则：凡是使用 AppleScript/Finder 自动化，必须同时具备 `automation.apple-events` entitlement + `NSAppleEventsUsageDescription`，并对 `-1743` 给出可执行的权限引导。

- 日期：2026-02-27
  - 实施纠正：只改 `Project.swift` 不重新生成工程会导致 InfoPlist 变更未生效。
  - 暴露问题：在 Tuist 项目中遗漏 `tuist generate`，验证时误判“配置已加但运行产物无该键”。
  - 预防规则：任何 `Project.swift` / target 配置变更后，必须先 `tuist generate` 再构建验证。

- 日期：2026-02-27
  - 用户纠正：单 Bundle 后 Finder 工具栏图标点击仍无反应，且语言入口在部分系统因顶部裁切不可见。
  - 暴露问题：在“单 Bundle + deep-link”改造时误删了 `reopen` 快开执行路径，导致仅扩展链路可触发打开；同时把语言入口放在标题区右上，受标题栏布局差异影响。
  - 预防规则：单 Bundle 快开必须至少保留两条独立触发链路（`open urls` 与 `reopen`）；可配置入口不要放在标题栏敏感区，优先放在内容区稳定位置。

- 日期：2026-02-27
  - 用户纠正：单 Bundle 后 Finder 工具栏点击“无响应”，且设置窗在新系统被标题栏上下挤压。
  - 暴露问题：移除 Launcher 后只保留了 `application(_:open:)` 路径，缺少“纯启动/重开”场景的快开逻辑；窗口安全区只靠视图 padding 不够稳。
  - 预防规则：单 Bundle 模式必须同时覆盖三类触发（open-url/launch/reopen），并在窗口层（styleMask + toolbar/titlebar）与视图层同时处理安全区。

- 日期：2026-02-27
  - 用户纠正：发布包应保持单 Bundle 体验，不接受 DMG 内双 App。
  - 暴露问题：此前把“无窗口快速执行”拆成独立 Launcher，虽然可用但增加了安装与理解成本。
  - 预防规则：当用户要求单 Bundle 时，必须同时收敛工程目标、Finder 回退链路、安装文案和打包脚本，避免“逻辑已单化但发布仍双包”的半改状态。

- 日期：2026-02-27
  - 用户纠正：macOS 26 上第三方终端唤起失败，不能只依赖单一 `bundle id -> open -b` 路径。
  - 暴露问题：LaunchServices 注册状态和应用安装位置在不同系统/分发渠道下不稳定。
  - 预防规则：终端唤起必须采用多级解析（bundle id、常见目录扫描、app 绝对路径），并优先使用绝对路径启动。

- 日期：2026-02-27
  - 用户纠正：顶部标题/语言入口在多系统都被遮挡，说明不是像素级问题。
  - 暴露问题：仅通过 SwiftUI padding 调整无法覆盖标题栏样式差异，需同时约束 NSWindow 样式。
  - 预防规则：遇到标题栏遮挡问题，必须同时处理视图布局和 `NSWindow.styleMask(.fullSizeContentView)` 层面的兼容。

- 日期：2026-02-27
  - 用户纠正：安装包场景下“未检测到 Launcher”提示不应默认引导去运行 Xcode scheme。
  - 暴露问题：未区分开发态与发布态，导致错误操作指引。
  - 预防规则：含开发环境操作的文案必须按运行态分流，发布态只给安装路径指引。

- 日期：2026-02-27
  - 用户纠正：安装后点击“一键添加到 Finder”提示缺失 Launcher。
  - 暴露问题：DMG 初版只打包主 App，遗漏了 PathBridgeLauncher。
  - 预防规则：只要工具栏入口依赖独立 Launcher，发布包必须同时交付并签名 Launcher。

- 日期：2026-02-27
  - 实施纠正：仅在项目级写 `DEVELOPMENT_TEAM` 不够直观，`xcodebuild` 仍可能显示 team 为空。
  - 暴露问题：签名关键配置没有固定到具体可签名 target，导致排查成本升高。
  - 预防规则：App/Launcher/Finder Extension 三个目标都显式写入同一 `DEVELOPMENT_TEAM`，并用 `xcodebuild -showBuildSettings` 回归确认。

- 日期：2026-02-27
  - 用户纠正：Finder 工具栏图标“颜色过重”，与系统工具栏风格不协调。
  - 暴露问题：图标设计只考虑品牌辨识度，忽略了工具栏 16pt 场景下的视觉权重。
  - 预防规则：新增图标时必须先在 Finder 工具栏小尺寸预览，默认采用低饱和、低对比方案。

- 日期：2026-02-27
  - 用户纠正：Tahoe 26 上设置面板顶部存在被遮挡风险。
  - 暴露问题：窗口内容区过紧且标题层级过大，没有为系统标题栏变化预留安全边距。
  - 预防规则：桌面设置窗默认预留顶部内边距，并把标题字号控制在紧凑窗口可承受范围内。
