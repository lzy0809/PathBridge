...
let iconsets: [(path: String, sourceCandidates: [String])] = [
    (
        "Apps/PathBridgeApp/Resources/Assets.xcassets/AppIcon.appiconset",
        [
            "docs/assets/icons/app-icon-source.png",
    ...
    (
        ...
        [
            "docs/assets/icons/launcher-icon-source.png",
            "build/design/reference/Go2Shell.iconset/icon_512x512@2x.png",
        ]
    ),
]
...
for iconset in iconsets {
    guard let sourcePath = iconset.sourceCandidates.first(where: { fileManager.fileExists(atPath: $0) }) else {
        throw NSError(
            domain: "icon-gen",
            code: 2,
            userInfo: [NSLocalizedDescriptionKey: "No icon source found for \(iconset.path)"]
        )
    }
    ...
    guard let sourceImage = NSImage(contentsOfFile: sourcePath) else {
        throw NSError(
            domain: "icon-gen",
            code: 3,
            userInfo: [NSLocalizedDescriptionKey: "Failed to load icon source: \(sourcePath)"]
        )
    }

    let iconsetPath = iconset.path
    let baseURL = URL(fileURLWithPath: iconsetPath, isDirectory: true)
    ...
    print("updated \(iconsetPath) from \(sourcePath)")
}
